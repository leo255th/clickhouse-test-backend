...
export class AppService {
  ...
  async addData(dto: DataDto): Promise<number> {
    console.log(dto);
    let sql = `INSERT INTO  test7(id) VALUES (${dto.id});`;
    sql += sql;
  ...
  async addCol(colName: string, colType: string): Promise<number> {
    const sql = `
    ALTER TABLE  test7 ADD COLUMN IF NOT EXISTS ${colName} ${colType};
    `;
    const r = await this.clickhouseService.query(sql);
    return r;
  }
  ...
  async addCols(): Promise<number> {
    ...
    sqlService.start();
    for (let i = 1; i <= 300; i++) {
      sqlService.addOneStatement(`ALTER TABLE  test7 ADD COLUMN IF NOT EXISTS ${'kgl' + i} Boolean;`);
    }
    for (let i = 1; i <= 300; i++) {
      sqlService.addOneStatement(`ALTER TABLE  test7 ADD COLUMN IF NOT EXISTS ${'mnl' + i} Float32;`);
    }
    for (let i = 1; i <= 300; i++) {
      sqlService.addOneStatement(`ALTER TABLE  test7 ADD COLUMN IF NOT EXISTS ${'yl' + i} Float32;`);
    }
    for (let i = 1; i <= 300; i++) {
      sqlService.addOneStatement(`ALTER TABLE  test7 ADD COLUMN IF NOT EXISTS ${'zd' + i} Float32;`);
    }
  ...
  randomValueSQL(): string {
    // const start = new Date().getTime();
    // 生成插入语句
    let insertSQL = 'INSERT INTO  test7';
    // 添加字段
    insertSQL += '(id,';
    for (let i = 1; i <= 300; i++) {
      insertSQL += ('kgl' + i)
